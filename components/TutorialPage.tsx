// ============================================================================
// REPLACES components/TutorialPage.tsx
//
// Only change from the current file: HTMLTutorial now picks an accent color
// based on tutorial.category. Risk & Decision Analysis tutorials render with
// the amber accent that matches RiskAnalysisPage; every existing GIS
// tutorial keeps the exact same emerald styling it has today — nothing
// about COMPONENT_MAP or the custom-component tutorials changes at all.
// ============================================================================
...
// Full, literal Tailwind class strings for each accent — written out in full
// (not interpolated) so Tailwind's build-time scanner picks up both variants.
const PROSE_EMERALD =
...
const PROSE_AMBER =
  `prose prose-slate max-w-3xl mx-auto
   prose-h1:text-3xl prose-h2:text-2xl prose-h3:text-xl
   prose-h2:mt-10 prose-h3:mt-8
   prose-a:text-amber-600 prose-a:font-semibold prose-a:underline
   hover:prose-a:text-amber-700
   prose-ul:list-disc prose-ul:pl-6
   prose-ol:list-decimal prose-ol:pl-6
   prose-li:my-2 prose-li:marker:text-amber-600
   prose-code:bg-slate-100 prose-code:text-amber-700
   prose-code:px-1 prose-code:py-0.5 prose-code:rounded
   prose-code:before:content-none prose-code:after:content-none
   prose-img:rounded-xl prose-img:mx-auto
   prose-img:max-w-full prose-img:my-10`;
...
function HTMLTutorial({ tutorial }: { tutorial: any }) {
  const isRisk = tutorial.category === RISK_CATEGORY;
  const accentHeaderText = isRisk ? "text-amber-300" : "text-emerald-300";
  const accentPlainText = isRisk ? "text-amber-600" : "text-emerald-600";
  const accentCode = isRisk ? "text-amber-300" : "text-emerald-300";
  const proseClasses = isRisk ? PROSE_AMBER : PROSE_EMERALD;
    ...
    <>
      {tutorial.image ? (
        ...
        >
          ...
          <div className="relative z-10 max-w-4xl mx-auto h-full flex flex-col justify-end px-6 pb-10">
            ...
            <p className={`${accentHeaderText} mt-2`}>
        ...
        <section className="max-w-4xl mx-auto pt-24 px-6">
          ...
          <p className={`${accentPlainText} mb-10`}>
            {tutorial.category} • {tutorial.level}
          </p>
        </section>
      )}
      ...
      <section className="max-w-4xl mx-auto py-20 px-6">
        ...
        {tutorial.codeSnippet && (
          <pre className={`bg-slate-900 ${accentCode} p-6 rounded-xl mt-12 overflow-x-auto text-sm`}>
